...
3. **`.cursor/skills/audit/SKILL.md` §4** — report format
...
**Depth:** slash default = HIGH/MEDIUM, cap **3**; trailing `full` → thorough + cap **4**; trailing `quick` (Matt only) → cap **2**. Trailing `audit-only` skips Phase 2. No plan in thread → report and stop.

Does **not** clear `/myauditandfix` push PENDING and does **not** unlock `git push`.

Orchestrator remains hub — dual critics + verifier per `loop.md`; do not solo-audit.
